...
export default function GeneralLineChart({setLoadingLineChart}) {
	...
	const getPivotData = async () => {
		const response = await fetch(`http://localhost:3000/get-pivot-data-${filter}`)
  ...
  const layout = {
	...
	return (
		<div style={{backgroundColor: '#fff', position: 'relative', borderRadius: '20px', overflow: 'hidden'}}>
			<div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', position: 'absolute', zIndex: 10, width: '100%'}}>
				<h3 style={{fontWeight: 600}}>Electricity output per {filter === 'd' ? 'Day' : (filter === 'w' ? 'Week' : 'Month')}</h3>
				<div>
					<select name="" id="lineChartFilter" style={{borderColor: '#c3d3e2', width: '150px', padding: '10px', cursor: 'pointer', outline: 'none', borderRadius: '10px'}} onChange={(e) => setFilter(e.target.value)}>
